...
import { V_Literal } from "./literals";
export class E_Array implements ScopeExpression {
  start: Number;
  end: Number;
  elements: ScopeExpression[];
  eval(this: E_Array, context: V_ValueHolder): V_Array {
    var builtElements:ScopeValue[]=[];
    builtElements=this.elements.map(x=>x.eval(context));
    var builtProps={};
    for(var i=0;i<builtElements.length;i++){
        builtProps[i]=[VariableType.VAR,builtElements[i]];
    }
    return new V_Array(context,builtProps);
  }
  constructor(elements?:Array<ScopeExpression>) {
    this.elements = elements?elements:new Array<ScopeExpression>();
  }
}
export class V_Array implements ScopeValue,V_ValueHolder {
    context: V_ValueHolder;
  base(this: V_Array): ScopeValue {
    return this;
  }
//   eval(this: V_Array, parameters: Array<ScopeExpression>, context: V_ValueHolder): ScopeValue {
...
//   }
  ...
  get(this: V_Scope, key: string): ScopeValue {
    if(key=="length"){
        ...
        return new V_Literal(Object.keys(this.properties).length)
      }
    var pList=this.parentList();
    if(this.has(key)){
      if(this.properties[key][1] instanceof V_Array){
        //console.log("Key",key);
        (this.properties[key][1] as V_Array).self=this;
      }
  ...
  set(this: V_Array,  type:VariableType, key: string, value: ScopeValue): ScopeValue {
    if(key=="action"){
  ...
  setAction(this: V_Array, value: ScopeValue): ScopeValue {
    var v=value.base();
  ...
  protoCopy(this:V_Array):V_Array{
    var cloneProps={};
    for (var key in this.properties) {
      cloneProps[key]=[this.properties[key][0],this.properties[key][1].base()];
    }
    var c=new V_Array(this.context,cloneProps,[],this.parameterMap);
    /*for (var key in c.properties) {
      if(c.properties[key][1] instanceof V_Array){

        //(c.properties[key][1] as V_Array).self=this;
      }
    }*/
    return c;
  }
